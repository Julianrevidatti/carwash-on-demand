...
class NotificationHelper(private val context: Context) {
    ...
    fun sendBookingConfirmationNotification(service: String, time: String) {
        sendStatusNotification(
            "¡Reserva confirmada!",
            "Tu lavado $service está programado para las $time.",
            100
        )
    }

    fun sendWasherOnWayNotification(washerName: String) {
        sendStatusNotification(
            "Lavador en camino",
            "$washerName está yendo hacia tu ubicación.",
            101
        )
    }
    ...
    fun sendStatusNotification(title: String, message: String, notificationId: Int) {
        val builder = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.mipmap.ic_launcher) // Usando el icono estándar del app
