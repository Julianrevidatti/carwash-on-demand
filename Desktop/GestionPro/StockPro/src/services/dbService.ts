...
export const productDB = {
    ...
    insert: async (p: any) => {
        await db().run(
            `INSERT INTO products (id, name, barcode, cost, profit_margin, price, supplier_id, is_pack, child_product_id, child_quantity, is_weighted, is_manual_price, image_url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [p.id, p.name, p.barcode || '', p.cost, p.profitMargin, p.price, p.supplierId || null, p.isPack ? 1 : 0, p.childProductId || null, p.childQuantity || null, p.isWeighted ? 1 : 0, p.isManualPrice ? 1 : 0, p.image_url || null]
        );
    },

    update: async (p: any) => {
        await db().run(
            `UPDATE products SET name=?, barcode=?, cost=?, profit_margin=?, price=?, supplier_id=?, is_pack=?, child_product_id=?, child_quantity=?, is_weighted=?, is_manual_price=?, image_url=? WHERE id=?`,
            [p.name, p.barcode || '', p.cost, p.profitMargin, p.price, p.supplierId || null, p.isPack ? 1 : 0, p.childProductId || null, p.childQuantity || null, p.isWeighted ? 1 : 0, p.isManualPrice ? 1 : 0, p.image_url || null, p.id]
        );
    },
